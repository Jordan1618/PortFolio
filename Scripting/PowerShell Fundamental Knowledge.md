...
- Stop-Service vector -ErrorAction SilentlyContinue ; Invoke-WebRequest -Uri "https://github.com/vectordotdev/vector/releases/download/v0.38.0/vector-0.38.0-x86_64-pc-windows-msvc.zip" -OutFile "$env:TEMP\vector_correct.zip" ; Expand-Archive "$env:TEMP\vector_correct.zip" -DestinationPath "$env:TEMP\vector_out" -Force ; Move-Item "$env:TEMP\vector_out\vector-x86_64-pc-windows-msvc\bin\vector.exe" "C:\vector\vector.exe" -Force ; Remove-Item "$env:TEMP\vector_out" -Recurse -Force
- 
