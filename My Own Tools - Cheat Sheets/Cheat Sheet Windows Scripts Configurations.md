...
7) ``Where-Object`` is a filter for a list and we use a {...} (script block) ; $_ is used to name the current read file ; .DisplayName means (.) go look for the DisplayName (variable or a name of line) ; -like is for "is it containing that ?" and "ASP.NET* "